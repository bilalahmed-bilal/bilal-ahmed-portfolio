...
import { developmentProcess } from "@/data/site";

export const metadata: Metadata = {
  title: "Development Process | From Idea to Production",
  description:
    "A structured development process covering discovery, architecture, development, testing, deployment, and continuous improvement.",
    ...
    <main id="main-content">
      <section className="border-b bg-grid">
        <div className="mx-auto max-w-6xl px-4 py-20 text-center sm:px-6 sm:py-24 lg:px-8">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-muted-foreground">
            Good engineering starts with clarity. Each stage exists to reduce risk and keep
            technical work aligned with the business outcome.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="space-y-4">
          {developmentProcess.map(([number, title, text]) => (
            <article
              key={number}
              className="grid gap-4 rounded-2xl border bg-card p-6 sm:grid-cols-[72px_190px_1fr] sm:items-center"
            >
      ...
      <section className="border-y bg-muted/30">
        <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8">
          <h2 className="text-center text-3xl font-black">Working principles</h2>

          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {principles.map((item) => (
              <div
                key={item}
                className="flex items-center gap-3 rounded-xl border bg-background p-4 text-sm font-semibold"
              >
